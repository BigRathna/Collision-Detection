...
def getTangentPoints(p,c):
    ...
    if d == 0 and r0 == c[2]:
        ...
        return [x3,y3],[x4,y4]
        # we check the angles 
        # l1=eqOfLine((p[0],p[1]),(x3,y3))
        # l1.append(angle(l[0],l1[0]))
        # l2=eqOfLine((p[0],p[1]),(x4,y4))
...
def allWaypointsVisited(wayP,visited):
    for i in wayP:
        if i not in visited:
            return False
    return True
...
while True:
    ...
    if curPt[2]=='c':
        ...
        for i in obs:
            if lineInCircle(l,i,curPt):
                p=footOfPerpendicular(l,i[0:2])
                if intOrExtDiv(curPt,nextPt,p)=="internal":
...
while True:
    ...
    for i in obs:
        ...
        # pygame.draw.circle(screen,red,i[0:2],d,width=2)
    # for i in bObs:
    #     pygame.draw.circle(screen,black,i[0:2],i[2],width=2)
