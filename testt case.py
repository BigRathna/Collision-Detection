...
def check_angle(t1,t2,r,point):
    ...
    pointx = point[0]
    pointy = point[1]
    angle_1 = math.tan(distance((tx1,ty1),(pointx,pointy))/r)
    angle_2 = math.tan(distance((tx2,ty2),(pointx,pointy))/r)
...
def check_direction(curr,next,obs_check):
    ...
    print(len(check_ptr),check_ptr)
    

    if(check_ptr == [] or len(check_ptr) == 1):
        return False
...
while curPt!=endPt:
    ...
    for i in obs:
        ...
        print(lineInCircle(l,i) , ( distance(i[0:2],curPt) < distance(curPt,nextPt) ) , check_direction(curPt,nextPt,i))
        
        if lineInCircle(l,i) and ( distance(i[0:2],curPt) < distance(curPt,nextPt) ) and check_direction(curPt,nextPt,i):
    ...
    # the error is most probably here instead of using waypoints array we should use pointsTDraw
        ...
        tangents = [[0,0],[0,0]]
        d=math.sqrt((curPt[0] - closestCirc[0])**2 + (curPt[1] - closestCirc[1])**2 )
        print(closestCirc[2]/d)
        angle = math.acos(closestCirc[2]/d)
        dir = math.atan2(closestCirc[1]-curPt[1],closestCirc[0]-curPt[0])
        dir1 = dir+ angle
        dir2 = dir - angle

        tangents[0][0] = closestCirc[0] + closestCirc[2] * math.cos(dir1)
        tangents[0][1] = closestCirc[1] + closestCirc[2] * math.sin(dir1)
        tangents[1][0] = closestCirc[0] + closestCirc[2] * math.cos(dir1)
        tangents[1][1] = closestCirc[1] + closestCirc[2] * math.sin(dir1)


        
        # Next point changes to point of tangent
        nextPt=check_angle(tangents[0],tangents[1],closestCirc[2],closestCirc[0:2])
        print(nextPt)
